...
docker run --name serv -p 5000:5000 -v /Users/AntonEfimenko/pgdata_back:/pgdata_back -e POSTGRES_PASSWORD=psw -e PGDATA_BACKUP=/pgdata_back/latest.psql.gz -e BACKUP_RESTORE_LOG=/pgdata_back/restore.log -e POSTGRES_USER=postgres -e POSTGRES_HOST=localhost -e POSTGRES_PORT=5432 -e POSTGRES_DB=postgres -d serv-restore
